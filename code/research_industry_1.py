...
def granger_test(university_df):
    ...
    series = university_df[['research_score', 'industry_income_score']].dropna()
    results = {'name': university_df['name'].iloc[0]}

    if len(series) > 3:
        for lag in range(1, 3):
            try:
                reversed_series = series[['industry_income_score', 'research_score']]
                ...
                results[f'ri{lag}_p'] = p_value1
                results[f'ri{lag}_F'] = f_stat1
                ...
                results[f'ir{lag}_p'] = p_value2
                results[f'ir{lag}_F'] = f_stat2
            except Exception:
                return None
    else:
        return None
...
def classify(row, lag):
    ri = row[f'ri{lag}_p'] < 0.05
    ir = row[f'ir{lag}_p'] < 0.05
    if ri and ir:
...
def classify_combined(row):
    ri = any(row[f'ri{lag}_p'] < 0.05 for lag in range(1, 3))
    ir = any(row[f'ir{lag}_p'] < 0.05 for lag in range(1, 3))
    if ri and ir:
        return 'Bidirectional'
    elif ri:
        return 'RI Causality'
...
def summarize(df, category_col):
    ...
    for cat in ['RI Causality', 'IR Causality', 'Bidirectional', 'No Causality']:
        ...
        if cat != 'Bidirectional':
            ...
            if category_col == 'category_combined':
                for lag in range(1, 3):
                    f_values += [v for v in subset[f'ri{lag}_F'].tolist() + subset[f'ir{lag}_F'].tolist() if isinstance(v, float)]
            else:
            ...
            if category_col == 'category_combined':
                for lag in range(1, 3):
                    f_bi_ri += [v for v in subset[f'ri{lag}_F'] if isinstance(v, float)]
                    f_bi_ir += [v for v in subset[f'ir{lag}_F'] if isinstance(v, float)]
            else:
                lag = int(category_col[-1])
                f_bi_ri = [v for v in subset[f'ri{lag}_F'] if isinstance(v, float)]
                f_bi_ir = [v for v in subset[f'ir{lag}_F'] if isinstance(v, float)]

            avg_ri, std_ri, med_ri, min_ri, max_ri = stat(f_bi_ri)
            avg_ir, std_ir, med_ir, min_ir, max_ir = stat(f_bi_ir)

            summary.append(['Bidirectional-RI', count, f"{perc}%", avg_ri, std_ri, med_ri, min_ri, max_ri,
                            avg_rank, std_rank, med_rank, min_rank, max_rank,
                            avg_score, std_score, med_score, min_score, max_score])
            summary.append(['Bidirectional-IR', count, f"{perc}%", avg_ir, std_ir, med_ir, min_ir, max_ir,
                            avg_rank, std_rank, med_rank, min_rank, max_rank,
                            avg_score, std_score, med_score, min_score, max_score])
...
# === Final Table Print ===
columns_order = ['name',
                 'ri1_p', 'ri1_F', 'ri2_p', 'ri2_F',
                 'ir1_p', 'ir1_F', 'ir2_p', 'ir2_F',
...
# === Mann-Whitney U Test ===
categories = ['RI Causality', 'IR Causality', 'Bidirectional', 'No Causality']
